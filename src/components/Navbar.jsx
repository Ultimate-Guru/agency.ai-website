...
const Navbar = ({ theme, setTheme }) => {
    ...
    return (
        <div className='flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70'>
            ...
            <a href="#">
                <img src={theme === 'dark' ? assets.logo_dark : assets.logo} alt="Agency Logo"
                className='w-32 sm:w-40' loading='lazy' />
            </a>

            <div className={`text-grey-700 dark:text-white sm:text-sm ${!sidebarOpen ? 'overflow-hidden max-sm:w-0' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
            ...
            <div className='flex items-center gap-2 sm:gap-4'>
                ...
                <a href="#contact-us" className='text-sm max-sm:hidden flex items-center gap-2 bg-[#5044E5] text-white px-6 py-2 rounded-full cursor-pointer hover:scale-103 transition-all'>
                    Connect
                    <img src={assets.arrow_icon} alt="Connect_Arrow" width={14} loading='lazy' />
                </a>
            </div>
